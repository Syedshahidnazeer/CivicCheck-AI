...
# Page Configuration
st.set_page_config(
    page_title="NGO Governance & Compliance Expert AI",
    page_icon="🏛️",
    layout="wide",
    initial_sidebar_state="expanded"
)

# --- 1. THE MASTER PROMPT (Embedded) ---
# We treat the provided JSON as the absolute source of truth for the model.
NGO_SYSTEM_PROMPT_JSON = """
{
  "agent_metadata": {
    ...
    "regulatory_knowledge_base": {
      ...
      "governance_and_audit_requirements": {
        ...
        "governance_audit_framework": {
          "board_structure": {
            "minimum_members": "3 for trust/society",
      ...
      "output_specifications": {
        "format_requirements": {
          "primary_format": "Detailed Markdown with clear hierarchical organization and Tables where appropriate.",
          "content_quality_standards": [
            "Regulatory Accuracy: Every reference verified against 2025 regulations",
            "Multi-Expert Integration: Seamless synthesis of 7 expert perspectives",
            "Actionable Specificity: Clear next steps, not theoretical concepts",
            "India-Contextual: All recommendations localized to Indian NGO landscape",
            "Implementation-Ready: Can be immediately used by NGO stakeholders",
            "Compliance-Focused: Risk areas flagged with mitigation strategies",
            "Completeness: Comprehensive coverage requiring no follow-up consultation"
          ],
          "delivery_expectation": "Production-ready consultation guidance representing expert-level, multi-disciplinary NGO advisory. Single response covers compliance, financial, operational, governance, strategic, and sector dimensions comprehensively."
        }
      },

      "forbidden_actions": [
        "DO NOT provide outdated regulatory guidance (pre-2025)",
        "DO NOT ignore FCRA May 2025 amendments or CSR July 2025 changes",
        "DO NOT treat Indian regulations as equivalent to international standards",
        "DO NOT provide legal advice—frame as regulatory guidance requiring legal verification",
        "DO NOT ignore compliance risks in favor of operational convenience",
        "DO NOT ask for NGO clarifications—make intelligent assumptions",
        "DO NOT deliver partial responses—comprehensiveness required",
        "DO NOT display individual expert deliberations—only final synthesis"
      ]
    }
  }
}
...
FULL_INSTRUCTION = f"""
You are a highly specialized AI agent acting as a Multi-Expert NGO Consultant for the Indian Sector. 
Your configuration, persona, knowledge base, and strict operational rules are defined in the JSON below. 
YOU MUST ADHERE STRICTLY TO THIS JSON CONFIGURATION.
Current Context Date: November 19, 2025.
...
with st.sidebar:
    ...
    st.markdown("Enter your Gemini API Key to activate the NGO Specialist.")
    ...
    st.divider()
    st.markdown("### ℹ️ Agent Capabilities")
    st.info("""
    **Roles Integrated:**
    - Chartered Accountant (CA)
    - Business Strategy Expert
    - NGO Operations Specialist
    - Indian Voluntary Sector SME
    ...
    **Regulatory Context:**
    - Income Tax Act 2025
    - FCRA May 2025 Amendments
    - CSR July 2025 Framework
    """)
...
st.title("🇮🇳 Indian NGO Governance & Compliance Specialist (2025)")
st.markdown("""
*This AI agent provides expert-level, one-shot comprehensive guidance for Indian NGOs, strictly adhering to the November 2025 regulatory framework (FCRA, CSR, Income Tax).*
""")
...
# React to user input
if prompt := st.chat_input("Describe your NGO scenario or regulatory query..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            # Create chat session (send history without system prompt as it's already configured)
            history_for_gemini = [
                {"role": "user" if m["role"] == "user" else "model", "parts": [m["content"]]}
                for m in st.session_state.messages[:-1] # Exclude current prompt which we send via send_message
            ]
            ...
            st.info("Tip: Ensure your API Key is valid and has access to the `gemini-1.5-flash` or `pro` model.")

# --- 4. DISCLAIMER ---
st.markdown("---")
st.caption("⚠️ **Disclaimer:** This AI operates based on a simulated regulatory framework for November 2025. While designed to be an expert system, it does not constitute legal counsel. Always verify critical compliance decisions with a practicing Chartered Accountant or Legal Professional.")